var mainApp = {};

(function(){
	var firebase = app_fireBase;
var uid = null;
	firebase.auth().onAuthStateChanged(function(user) {
  		if (user) {
    		// User is signed in.
    		uid = user.uid;
  		}else{
  			// Redirect to login page
  			uid = null;
  			window.location.replace("login.html");
  		}
	  });

	function logOut(){
		firebase.auth().signOut();
	}

	function reload_page(){
		window.location.reload();
	   }


	function messageHandler(err){
		if(!err){
			console.log(err)
		}else{
			console.log("success");
		}
	}
	function checkforblank(){
		if(document.getElementById('items_id').value == ""){
			alert('Please Enter an Item ID!');
			document.getElementById('items_id').style.borderColor ="red";
			return false;
		}
	}

	function fnCreate(){
		var path = 'users/' + uid;
		var data = {
			name: "Gabriel",
			age: 29,
			message: "Hello firebase"
		}
		app_fireBase.databaseApi.create(path, data, messageHandler);

	}
	function fnRead(){
		var path = 'users/' + uid;
		app_fireBase.databaseApi.read(path, successFn, messageHandler);
		function successFn(snapShot){
			if(!!snapShot){
				console.log(snapShot.val());
			}else{
				console.log('No data found');
			}

		}
	}



	function fnDelete(itemID){
		var path = 'items/' + itemID;
		app_fireBase.databaseApi.delete(path, messageHandler);
		alert('The submission was deleted successfully!');
		reload_page();
	}
	
	function fnDecline(){
		var item_id = document.getElementById('items_id').value;
		var path = 'items/'+ item_id +'/';
		
		var data = {
			approvalStatus: 2
		}
		app_fireBase.databaseApi.update(path, data, messageHandler);
		alert('The submission was declined successfully!');
   		reload_page();
	}
	function fnApprove(){
		var item_id = document.getElementById('items_id').value;
		var path = 'items/'+ item_id +'/';
		
		var data = {
			approvalStatus: 1
		}
		app_fireBase.databaseApi.update(path, data, messageHandler);
		alert('The submission was approved successfully!');
   		reload_page();
	}

	





	var user = firebase.auth().currentUser;
  	var tblUsers = document.getElementById('tbl_items_list');
	var databaseRef = firebase.database().ref('items/').orderByChild("approvalStatus").equalTo(0);
	
  	var rowIndex = 1;
  
  	databaseRef.once('value', function(snapshot) {
    snapshot.forEach(function(childSnapshot) {
   	var childKey = childSnapshot.key;
	   var childData = childSnapshot.val();
	   

   
	   var row = tblUsers.insertRow(rowIndex);
	   var cellId = row.insertCell(0);
   	   var cellName = row.insertCell(1);
	   var cellDescription = row.insertCell(2);
	   var cellLocation = row.insertCell(3);
	   var cellStatus = row.insertCell(4);
	   var cellPoster = row.insertCell(5);
	
   	   cellId.appendChild(document.createTextNode(childData.itemID));
	   cellName.appendChild(document.createTextNode(childData.itemName));
	   cellDescription.appendChild(document.createTextNode(childData.description));
	   cellLocation.appendChild(document.createTextNode(childData.locationDescription));
	   cellStatus.appendChild(document.createTextNode(childData.status));
	   cellPoster.appendChild(document.createTextNode(childData.poster));
	   var cellButtons = row.insertCell(6).outerHTML= "<td><button type='button' id='row' value='"+childKey+"' class='decline btn btn-primary center-block' onclick='fnUpdate(this.value)'>SELECT</button></td>";
   	
	 rowIndex = rowIndex + 1;
	   
	  
    	});
 	 });


	mainApp.Create = fnCreate;
	mainApp.Read = fnRead;
	mainApp.Delete = fnDelete;
	mainApp.logOut = logOut;
	mainApp.Check = checkforblank;
	mainApp.Decline = fnDecline;
	mainApp.Approve = fnApprove;



})()